...
export default function BlogSidebar({
  ...
  const categories: { id: CategoryType; name: string; icon: string; count: number }[] = [
    { id: 'insurance', name: '4대사회보험', icon: '🛡️', count: posts.filter(p => p.category === 'insurance').length },
    { id: 'wage', name: '급여·노무', icon: '⏱', count: posts.filter(p => p.category === 'wage').length },
    { id: 'finance', name: '금융·재테크', icon: '💰', count: posts.filter(p => p.category === 'finance').length },
    { id: 'property', name: '부동산·세금', icon: '🏠', count: posts.filter(p => p.category === 'property').length },
    { id: 'life', name: '생활·행정', icon: '🎂', count: posts.filter(p => p.category === 'life').length },
  ];
  ...
  return (
    <aside className="space-y-6">
      ...
      {onOpenAutoPoster && (
        <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-slate-900 text-white rounded-2xl p-5 border border-indigo-500/30 shadow-md space-y-3.5">
          ...
          <p className="text-xs text-slate-300 leading-relaxed bg-slate-800/60 p-2.5 rounded-xl border border-slate-700/50">
            매일 5개 카테고리별 1회씩 4시간 텀 랜덤 시분초에 AI가 최적화 글을 자동 생성 및 배포합니다.
          </p>

          <div className="flex items-center justify-between pt-1">
            <span className="text-[11px] text-slate-400">
              자동 누적: <strong className="text-indigo-300 font-num">{autoGeneratedCount}</strong>편
            </span>
            <button
              type="button"
              onClick={onOpenAutoPoster}
              className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-3 py-1.5 rounded-xl transition flex items-center gap-1 shadow-sm cursor-pointer"
            >
              <span>스케줄 관리</span>
