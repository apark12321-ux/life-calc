...
export default function QualityCalculatorHub({ category, subCalculatorId = 'all' }: { category: CategoryType; subCalculatorId?: string }) {
  ...
  const counts = useMemo(() => Object.fromEntries(categoryKeys.map((key) => [key, calculatorCatalog.filter((item) => item.category === key).length])) as Record<string, number>, []);
    ...
    <section className="bg-white border border-slate-100 shadow-xs rounded-none sm:rounded-2xl px-3 py-4 sm:p-6 lg:p-8 overflow-hidden">
      <div className="border-b border-slate-100 pb-4 sm:pb-6 mb-4 sm:mb-6">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4 sm:gap-5">
          <div className="min-w-0">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 text-blue-700 px-2.5 sm:px-3 py-1 text-[11px] sm:text-xs font-extrabold mb-2 sm:mb-3"><ListChecks className="w-3.5 h-3.5 shrink-0" /><span>검수형 계산기 {calculatorCatalog.length}종</span></div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-black tracking-tight text-slate-900 leading-tight">생활계산기 천국</h1>
            <p className="text-[13px] sm:text-sm text-slate-500 mt-2 max-w-3xl leading-relaxed break-keep">자주 찾는 생활 계산기를 주제별로 모았습니다. 계산기마다 입력 항목, 계산식, 결과 항목이 다르게 구성됩니다.</p>
          </div>
          <div className="-mx-1 px-1 overflow-x-auto scrollbar-none xl:overflow-visible xl:mx-0 xl:px-0"><div className="flex xl:flex-wrap gap-2 min-w-max xl:min-w-0 xl:justify-end"><div className="min-w-[76px] rounded-xl bg-slate-900 text-white px-3 py-2 text-center"><p className="text-base sm:text-lg font-black leading-none">{calculatorCatalog.length}</p><p className="text-[10px] text-slate-300 font-bold mt-1">전체</p></div>{categoryKeys.map((key) => <div key={key} className="min-w-[76px] rounded-xl bg-slate-50 border border-slate-100 px-3 py-2 text-center"><p className="text-base sm:text-lg font-black leading-none text-slate-900">{counts[key] || 0}</p><p className="text-[10px] text-slate-500 font-bold mt-1 whitespace-nowrap">{categoryLabels[key]}</p></div>)}</div></div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(360px,430px)] gap-4 sm:gap-6 items-start">
        <aside id="calc-result-panel" className="order-1 lg:order-2 bg-slate-900 rounded-2xl p-4 sm:p-5 text-white shadow-lg h-fit lg:sticky lg:top-4 print-result scroll-mt-20">
          ...
          <div className="mt-3 rounded-2xl bg-white/10 border border-white/10 p-3 text-[12px] leading-relaxed text-slate-200"><p className="font-black text-white mb-1">계산식</p><p className="break-keep">{selected.formula}</p><p className="mt-2 text-slate-300 break-keep">{selected.note}</p></div>
