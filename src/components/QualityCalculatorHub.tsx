...
export default function QualityCalculatorHub({ category, subCalculatorId, onNavigateToCalculator }: Props) {
  ...
  const defaultResult = calculate(selected, defaultValues(selected));
  ...
  const related = calculatorCatalog.filter((item) => item.category === selected.category && item.id !== selected.id).slice(0, 4);
    ...
    <section className="bg-white border border-slate-100 shadow-xs rounded-none sm:rounded-2xl px-3 py-4 sm:p-6 lg:p-8 overflow-hidden">
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(360px,430px)] gap-4 sm:gap-6 items-start">
        <aside id="calc-result-panel" className="order-1 lg:order-2 bg-slate-900 rounded-2xl p-4 sm:p-5 text-white shadow-lg h-fit lg:sticky lg:top-4 print-result scroll-mt-20">
          <div className="flex items-start gap-3 border-b border-white/10 pb-4 mb-4"><div className="w-11 h-11 rounded-2xl bg-white/10 flex items-center justify-center text-2xl shrink-0">{selected.icon}</div><div className="min-w-0"><p className="text-[11px] font-black text-blue-200">{categoryLabels[selected.category]}</p><h2 className="text-lg font-black leading-tight mt-1 break-keep">{selected.name}</h2><p className="text-xs text-slate-300 mt-1 leading-relaxed break-keep">{selected.description}</p></div></div>
          <section className="print-hide"><h2 className="text-sm font-black text-white mb-3">입력 항목</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">{selected.fields.map((field) => <label key={field.key} className="block"><span className="block text-[13px] font-bold text-slate-300 mb-1.5">{field.label}{field.unit ? ` (${field.unit})` : ''}</span><input type={field.type || 'number'} inputMode={(field.type || 'number') === 'number' ? 'decimal' : undefined} value={String(values[field.key] ?? field.defaultValue)} onChange={(event: any) => setValues((prev) => ({ ...prev, [field.key]: (field.type || 'number') === 'number' ? Number(event.target.value) : event.target.value }))} className="w-full min-h-11 rounded-xl border border-white/10 bg-white/10 px-3 py-3 text-right text-base sm:text-sm text-white font-bold focus:outline-hidden focus:ring-2 focus:ring-blue-400" /></label>)}</div></section>
          <section className="mt-5"><h2 className="text-sm font-black text-white mb-3">계산 결과</h2><div className="rounded-2xl bg-white text-slate-900 p-3 sm:p-4 space-y-2 print-result-box">{result.map((item) => <div key={item.label} className={`flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 ${item.highlight ? 'bg-blue-50' : 'bg-slate-50'}`}><span className="text-[13px] font-bold text-slate-500">{item.label}</span><span className={`text-base sm:text-sm font-black text-right break-keep ${item.highlight ? 'text-blue-700' : 'text-slate-800'}`}>{item.value}</span></div>)}</div></section>
          <section className="mt-3 rounded-2xl bg-white/10 border border-white/10 p-3 text-[12px] leading-relaxed text-slate-200"><h2 className="font-black text-white mb-1">계산 공식</h2><p className="break-keep">{selected.formula}</p><h2 className="font-black text-white mt-3 mb-1">주의사항</h2><p className="text-slate-300 break-keep">{selected.note}</p></section>
          <div className="grid grid-cols-2 gap-2 mt-4 print-hide"><button onClick={() => setValues(defaultValues(selected))} className="min-h-11 inline-flex items-center justify-center gap-2 rounded-xl bg-white/10 hover:bg-white/15 border border-white/10 px-3 py-3 text-[13px] font-black transition"><RotateCcw className="w-4 h-4" /> 초기화</button><button onClick={() => window.print()} className="min-h-11 inline-flex items-center justify-center gap-2 rounded-xl bg-blue-500 hover:bg-blue-400 px-3 py-3 text-[13px] font-black transition">결과 인쇄 <ArrowRight className="w-4 h-4" /></button></div>
        </aside>
